...
run_one_exp()
{
    ...
    for nthreads in 1024; do
        ...
        for accessmode in "uniform" "zipfian"; do
            ...
            # /users/lyuze/Pond/redis/ycsb
            local R_YCSB_LOAD_CMD="cd /users/lyuze/Pond/redis/ycsb; mkdir -p ${output_dir}; $CPREFIX -- ./bin/ycsb load redis -s -P workloads/${w} -P ${REDIS_RUN_DIR}/redis-load2.properties > ${loadoutputf} 2>&1"
            ...
            if [[ "${RUN_DAMON}" == 1 ]]; then
                local R_YCSB_RUN_CMD="cd /users/lyuze/Pond/redis/ycsb; mkdir -p ${output_dir}; ($CPREFIX -- ./bin/ycsb run redis -P workloads/${w} -P ${REDIS_RUN_DIR}/redis-run2.properties -p redis.host=${REDIS_SERVER} -p requestdistribution=${accessmode} -p threadcount=${nthreads} -p measurement.raw.output_file=${rawlatf} >${outputf} 2>&1 &); sleep 1; ycsb_pid=\$(ps -ef | grep java | grep -v grep | awk '{print \$2}'); sudo $DAMON record -s 1000 -a 100000 -u 1000000 -n 1024 -m 1024 -o $damonf \$ycsb_pid >/dev/null 2>&1"
            else
                local R_YCSB_RUN_CMD="cd /users/lyuze/Pond/redis/ycsb; mkdir -p ${output_dir}; $CPREFIX -- ./bin/ycsb run redis -P workloads/${w} -P ${REDIS_RUN_DIR}/redis-run2.properties -p redis.host=${REDIS_SERVER} -p requestdistribution=${accessmode} -p threadcount=${nthreads} -p measurement.raw.output_file=${rawlatf} >${outputf} 2>&1"
            fi
